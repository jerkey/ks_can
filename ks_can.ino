...
void send506() {
  command   = ZERO_BMS_CONTROL_CODE_NOT_SAFETY_OVERRIDE + ZERO_BMS_CONTROL_CODE_DISCONNECT_MODULE + ZERO_BMS_CONTROL_CODE_OPEN_CONTACTOR; // charger 1
  //command = ZERO_BMS_CONTROL_CODE_NOT_SAFETY_OVERRIDE + ZERO_BMS_CONTROL_CODE_KEY_ON + ZERO_BMS_CONTROL_CODE_DISCONNECT_MODULE + ZERO_BMS_CONTROL_CODE_OPEN_CONTACTOR + ZERO_BMS_CONTROL_CODE_OPEN_FET; // first CAN 1
  num_modules = 0;
  //zero_control_tail = 0xFFFF; // send FF's for last two bytes (not when charging though?)
  if (millis() > 5000) {
    command += ZERO_BMS_CONTROL_CODE_CHARGER_CONNECTED; // charger 2
    //command = ZERO_BMS_CONTROL_CODE_NOT_SAFETY_OVERRIDE + ZERO_BMS_CONTROL_CODE_KEY_ON + ZERO_BMS_CONTROL_CODE_CONNECT_MODULE  + ZERO_BMS_CONTROL_CODE_OPEN_CONTACTOR + ZERO_BMS_CONTROL_CODE_CLOSE_FET; // first CAN 2
  }
  if (millis() > 10000) {
    num_modules = 1;
    //command = ZERO_BMS_CONTROL_CODE_NOT_SAFETY_OVERRIDE + ZERO_BMS_CONTROL_CODE_KEY_ON + ZERO_BMS_CONTROL_CODE_CONNECT_MODULE  + ZERO_BMS_CONTROL_CODE_CLOSE_CONTACTOR + ZERO_BMS_CONTROL_CODE_CLOSE_FET; // first CAN 3
    command = ZERO_BMS_CONTROL_CODE_NOT_SAFETY_OVERRIDE + ZERO_BMS_CONTROL_CODE_CHARGING_ENABLED + ZERO_BMS_CONTROL_CODE_CHARGER_CONNECTED + ZERO_BMS_CONTROL_CODE_CONNECT_MODULE + ZERO_BMS_CONTROL_CODE_OPEN_CONTACTOR + ZERO_BMS_CONTROL_CODE_CLOSE_CONTACTOR; // charger 3
  }
